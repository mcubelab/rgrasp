...
double Quaternion::dist(const Quaternion &original) const
{
  double d1 = (*this - original).norm();
  double d2 = (*this + original).norm();
  if (d1 <= d2)
    return d1;
  return d2;
}
...
Quaternion Quaternion::angle2quat(double r1, double r2, double r3)
{
  ...
  for (int i=0;i<3;i++)
    {
      cang[i] = cos(angles[i]/2);
      sang[i] = sin(angles[i]/2);
    }
...
void Quaternion::quat2angle(double &r1, double &r2, double &r3)
{
  this->normalize();

  double r11 = 2*(v[1]*v[2] + v[0]*v[3]);
  double r12 = pow(v[0],2) + pow(v[1],2) - pow(v[2],2) - pow(v[3],2);
  double r21 = -2*(v[1]*v[3] - v[0]*v[2]);
  double r31 = 2*(v[2]*v[3] + v[0]*v[1]);
  double r32 = pow(v[0],2) - pow(v[1],2) - pow(v[2],2) + pow(v[3],2);

  threeAxisRot(r11, r12, r21, r31, r32, r1, r2, r3);
}

void Quaternion::threeAxisRot(double r11, double r12, double r21, 
