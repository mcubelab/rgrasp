...
def case_generator(binNum, listener, br):
    # 1. load object names
    obj_list = read_objlist()
    ...
    distribution_obj = [1, 2, 2, 2, 3, 3 , 4, 4, 4, 5, 7, 7] 
    distribution_obj = [1, 2, 2, 2, 2 , 2, 2, 2, 2, 2, 2, 2] 
        ...
        for attempt in range(num_attempts):
            ...
            if is_possible == True:
                ...
                for it_obj in range(num_obj):
                    ...
                    object_pose_map = poseTransform(obj_pose, 'bin'+str(binNum), '/map', listener)
                    object_pose[1] = object_pose_map
                    score = random.uniform(0.0 , 1.0)
                    object_pose['label'] = object_pose[0]
                    object_pose['pose'] = object_pose_map
                    object_pose['pca'] = object_pose_map
                    object_pose['latent'] = get_obj_dim(obj)
                    object_pose['mean'] = object_pose_map[0:3]
                    object_pose['rangeX'] = [object_pose_map[0]-0.1, object_pose_map[0]+0.1]
                    object_pose['rangeY'] = [object_pose_map[1]-0.1, object_pose_map[1]+0.1]
                    object_pose['rangeZ'] = [object_pose_map[2]-0.05, object_pose_map[2]+0.05]
                    object_pose['score'] = score
                    obj_ids.append(obj_in_bin[it_obj])
                    object_poses.append(object_pose)
                    visualizeObjectsDict(object_poses , br, 'map', rgba)
...
if __name__=='__main__':
    ...
    for binNum in xrange(0, 12):
        result = case_generator(binNum=binNum, listener=listener, br=br)
        
        letter = chr(binNum + ord('A'))
        objects_in_shelf["bin_%s" % (letter)] = result["objects_id"]
        pose_in_shelf["bin_%s" % (letter)] = result["poses"]
        work_order.append({'bin': "bin_%s" % (letter), 'item': result["objects_id"][0]})

    
    outfilename = 'apc_pick_task.json' 
    
    
    with open(outfilename, 'w') as outfile:
        json.dump({'bin_contents': objects_in_shelf, 'tote_contents': [], 'work_order': work_order} , outfile, sort_keys = True, indent = 4, ensure_ascii=False)


    outfilename = 'apc_pick_task.pose.json'
    with open(outfilename, 'w') as outfile:
        json.dump({'bin_contents': pose_in_shelf, 'tote_contents': []} , outfile, sort_keys = True, indent = 4, ensure_ascii=False)
