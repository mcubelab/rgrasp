...
class PlacingPlanner(object):
    ...
    def get_local_scores(self, obj, HeightMap=0, b=0):
        ...
        if in_self and not self.available[b]:  #If bin is not available, assign high scores
            #self.e.db('Bin ',b+1,' not available')
            Scores = 5.*self.INF*np.ones([len(HeightMap), len(HeightMap[0])])
            return Scores, [self.best_local_score, b, best_x, best_y, obj.theta, 1000]
        ...
        '''
        self.e.db('object dims', obj.w_dim)
        self.e.db(dims=obj.w_dim)
        self.e.db(b=b)
        self.e.db(T=type(Scores))
        '''
        if best_x == -1:
            self.e.db('The object dimensions: ', obj.w_dim, ' are probably very large when orientation is: ', obj.theta, '. Placing in the center of bin:', b+1)
        ...
        self.e.db('best score: ',self.best_local_score, ' in bin: ', b+1, ' is at (x,y): ', best_x, best_y, 'for theta ', obj.theta)
        return Scores, [self.best_local_score, b, best_x, best_y, obj.theta, max_h]
    ...
    def get_best_local_positions(self, obj, HeightMap=0, b=0, k=5):
        ...
        for x in range(len(Scores)):
            for y in range(len(Scores[x])):
                if Scores[x][y] == self.best_local_score: 
                    candidates.append([Scores[x][y], b, x, y, obj.theta, max_h]) #TODO: max_h
        candidates.sort()
    ...
    def place_object_local_best(self, obj=0, containers = None, undesired_containers = []):
        ...
        self.h_max = C[0][5]
        '''
        self.e.db(Limit_x = self.limit_x)
        self.e.db(Limit_y = self.limit_y)
        self.e.db(x = x)
        self.e.db(y = y)
        self.e.db(AssignedPos=obj.pos)
        self.e.db(AssignedTheta=obj.theta)
        self.e.db(AssignedScore=C[0][0])
        '''
    ...
    def place_object_choosebin(self, obj=0, containers = None):
        ...
        self.e.db('weightScore: ',weightScore)
    ...
    def put_objects_from_input(self): #Only used in main
        ...
        while not ok:
            ...
            try:
                ...
                ok = False
            if self.n <= 0:
                self.e.db('Too few objects')
                ok = False
            elif self.n > 50:
                self.e.db('Too many objects')
                ok = False
        self.objects = []
    ...
    def update_real_height_map(self, b = None, img_path = None):
        ...
        if self.available[b]:
            ...
            # while long(self.passive_vision_state.heightmap_timestamp) < self.time_since_clean[b+1]:
                # self.e.db('Vision is not yet updated, clean time according planner: ', self.time_since_clean[b+1], ' according vision: ', self.passive_vision_state.heightmap_timestamp)
                # time.sleep(0.5)
            ...
            with Timer('Call passive vision to request new height map for %d' % (b+1)):
                self.e.db('getPassiveVisionEstimate ', 'request', '',  b+1)
            ...
            if len(M) > 0:
                ...
                self.e.db('There is no HM, we will be using the old one....')
                return
        else:
    ...
    def add_object_to_height_map(self, obj, HeightMap=0): #Only used in virtual mode for vision.
        ...
        '''
        self.e.db(limits=limits)
        self.e.db(w_dim=obj.w_dim)
        self.e.db(pos=obj.pos)
        self.e.db(b=b)
        '''
        ...
        #self.e.db(new_height=new_height)
    ...
    def update_pose_placed_obj(self, obj):
        ...
        #convert obj.theta into quaternion:
        orient_mat_3x3 = rotmatZ(obj.theta*180.0/np.pi) 
        obj.pose[3:7] = mat2quat(orient_mat_3x3)
        self.e.db('Added object:', obj.label,' to HM considering obj.pos: ', obj.pos, ', bin center: ', center_bin, 'and obj.pose: ', obj.pose, ' in obj.container: ', obj.container)
        return obj
