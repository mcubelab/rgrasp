...
target_pose  = [[1.50, 0, 0.90,  0, 0.7071, 0, 0.7071],
                [1.50, 0, 0.85,  0, 0.7071, 0, 0.7071],
                [1.55, 0, 0.85,  0, 0.7071, 0, 0.7071],
                [1.55, 0, 0.85,  0, 0.7071+5.0*3.1415/180, 0, 0.7071-5.0*3.1415/180]]
...
qf = None
for counter in range(0,len(target_pose)):
    planner = IK(q0 = qf, target_tip_pos = target_pose[counter][0:3], target_tip_ori = target_pose[counter][3:7], joint_topic=joint_topic, tip_hand_transform=tip_hand_transform) 
    plan = planner.plan()
...
br.sendTransform(tuple(target_pose[0][0:3]), tuple(target_pose[0][3:7]), 
...
if user_reply == 'y':
    ...
    for count in range(0, len(plans)):
        pause()
        plans[count].execute()
        
    for count in range(1,len(plans)):
        back_count = len(plans)-count
        pause()
        plans[back_count].executeBackward()
