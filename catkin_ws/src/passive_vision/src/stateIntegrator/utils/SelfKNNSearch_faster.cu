__global__ void SelfKNNSearch_faster( int * result, const int * args, const float * pc1, const float * pc_organized, const int * unorg2org, const int * org2unorg)
{
    ...
    if (pc1Idx < pc1NumPts) {
      ...
      nn_dists[0] = 0.0;
      nn_index[0] = pc1Idx;
      int pc1Idx_org = unorg2org[pc1Idx];
      int pc1x = pc1Idx_org / 
