...
def scoop(objPose = [1.95,0.25,1.4,0,0,0,1],
    ...
    planner = IK(q0 = q_initial, target_tip_pos = targetPosition, target_tip_ori = scoopOrientation, tip_hand_transform=tip_hand_transform, joint_topic=joint_topic, useMoveit=useMoveit)
    plan = planner.plan()
...
def main(argv=None):
    ...
    parser.add_option('-f', '--full', action="store_true", dest='doFullTest', help='Going into each bin', 
                      default=False)
                      
    parser.add_option('-n', '--nopause', action="store_false", dest='withPause', help='No pause between executions', 
                      default=True)
                      
    parser.add_option('-m', '--moveit', action="store_true", dest='useMoveit', help='Use moveit', 
                      default=False)
    ...
    objPoses = []
    objPoses.append([1.56, 0.450, 1.11,0,0,0,0])
    objPoses.append([1.56, -0.450, 0.91,0,0,0,0])
    objPoses.append([1.56, 0.450, 0.71,0,0,0,0])
    objPoses.append([1.56, -0.450, 0.51,0,0,0,0])
    
    yy = [0.25, 0, -0.25]   # at the middle of each bin for each row
    deltay = -0.04 # from shelf calibration
    ...
    if opt.doFullTest:
        ...
        for binNum in binNums:
            (overall_plan_succeed,suction_succeed) = scoop(
                objPose = [1.6, yy[binNum % 3] + deltay, 1.1, -0.0030, 0.6979, -0.7161, 0.0023],
                binNum = binNum, 
                robotConfig=None, 
                isExecute = True,
                withPause = opt.withPause,
                useMoveit = opt.useMoveit)
            if not overall_plan_succeed:
                testFailResult.append((binNum))
        print 'testFailResult', testFailResult
    else:
        binNums = [0,3,6,9]
        for x in range(0,4):
