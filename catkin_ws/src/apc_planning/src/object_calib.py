...
import math
import yaml
from ik.ik import setCart, setSpeed, setAcc
...
def object_calib(object_id = 'shelf_icp', nmeasure = 10):
    ...
    origJoint = getJoint()
    #pause()
    measured_shelfz = -0.524125337601
    ...
    for tag_obj in tags_object_poses:
        ...
        for i in xrange(nmeasure): 
            ...
            bestRightPoint = (0,0,0)
            bestLeftPoint = (0,0,0)
            
            # import json
            # with open('data%d_%d_old.txt' % (tag_obj_id,i), 'w') as outfile:
                # json.dump(pointcloud.cloudXYZ, outfile)
            # with open('data%d_%d_T_old.txt' % (tag_obj_id,i), 'w') as outfile:
            ...
            if tag_obj_id == 0 and abs(bestRightPoint[0]) > 1e-9:
                bestRightPoints.append(bestRightPoint)
            elif tag_obj_id == 1 and abs(bestLeftPoint[0]) > 1e-9:
                bestLeftPoints.append(bestLeftPoint)
