...
bool Vec::operator < (const Vec &orig) const
{
  return ((v[0]<orig[0]) || 
	  ((v[0]==orig[0]) && (v[1]<orig[1])) || 
...
{
  double t = 0.0000001;
  return ((fabs(v[0]-orig[0])<t) && (fabs(v[1]-orig[1])<t) && (fabs(v[2]-orig[2])<t));
}

bool Vec::in(double elem) const
{
...
bool Vec::parallel(const Vec &orig) const
{
  Vec v1 = Vec(*this);
  Vec v2 = Vec(orig);
  v1.normalize();
