...
void CreateSensorMessage(EgmSensor* pSensorMessage, int root_x, int root_y)
{
    ...
    pc->set_x(305+root_x/30);
    pc->set_y(-0.33+root_y/30);
    pc->set_z(500);
    printf("%d %d\n", root_x, root_y);
...
// 
    ...
    while(1)
    {
      try{
        ...
        XQueryPointer(dpy, root, &ret_root, &ret_child, &root_x, &root_y,
					 &win_x, &win_y, &mask);
        CreateSensorMessage(pSensorMessage, root_x, root_y);
