...
class TaskPlanner(object):
    def __init__(self, args):
        ...
        self.FAKE_SUCTION_DIR = os.environ['CODE_BASE'] + '/input/fake_dirs/fake_suction/'
        ...
        if not os.path.isfile(self.jsonfilename):
            self.jsonfilename = os.environ['CODE_BASE'] + '/input/' + self.jsonfilename
        if not os.path.isfile(self.json_order_filename):
            self.json_order_filename = os.environ['CODE_BASE'] + '/input/' + self.json_order_filename
        if not os.path.isfile(self.interactive_filename):
            self.interactive_filename = os.environ['CODE_BASE'] + '/input/' + self.interactive_filename
        if not os.path.isfile(self.json_boxes_filename):
            self.json_boxes_filename = os.environ['CODE_BASE'] + '/input/' + self.json_boxes_filename
...
#        self.webpage = webpages.WebDisplay()
    ...
    def enter_jsonfile(self):
        ...
        if not self.use_last_json and self.task=='stowing': #TODO: change to allow input from picking
            ...
            with open(os.path.join(os.environ['CODE_BASE'], 'input/last_input.json'), 'w') as outfile:
    ...
    def enter_jsonfile_boxes(self):
            ##Introduce boxes
            if self.json_boxes_filename != os.environ['CODE_BASE'] + '/input/':  #No file name assigned to it
    ...
    def enter_jsonfile_goals(self):
            ...
            if self.json_order_filename != os.environ['CODE_BASE'] + '/input/':  #No file name assigned to it
...
###############
    ...
    def pickingPreparation(self):
        ...
        outfilename = os.environ['CODE_BASE']+'/input/interactive_placing.json'
    ...
    def json_output(self, interactive_file = False):
        if interactive_file:
            self.interactivefilename = os.environ['CODE_BASE']+'/input/interactive_result.json'
        if self.task == 'stowing':
            self.outfilename = os.environ['DATA_BASE']+'/stow_result/stow_result.json'
            self.outfilename_in_output = os.environ['CODE_BASE']+'/output/stow_result.json'
            #Add all objects in bin 1
            self.outfilename_complete = os.environ['DATA_BASE']+'/stow_result_complete/stow_result_complete.json'
            self.outfilename_complete_in_output = os.environ['CODE_BASE']+'/output/stow_result_complete.json'
        else:
            self.outfilename = os.environ['DATA_BASE']+'/pick_result/pick_result.json'
            self.outfilename_in_output = os.environ['CODE_BASE']+'/output/pick_result.json'
        DATA = {}
        DATA['bins'] = []
        for x in range(self.num_bins):
            DATA['bins'].append({'bin_id': chr(ord('A')+x), 'contents': self.shelfObjNamesGivenBin(x+1) })
    ...
    def talkWithHumanToStartExperiment(self):
        ...
        if last_input == 'y':
            ...
            if last_input == 'y':
                ...
                self.jsonfilename = os.path.join(os.environ['CODE_BASE'], 'input/last_input.json')
                self.enter_jsonfile()
                return
            else:
                e.db('I thought so')
    ...
    def init_experiment(self, args):
        ...
        #Vision Logs
        self.data_directory = os.environ['DATA_BASE']+'/loopdata/planner/{}'.format(int(round(time.time())))
    ...
    def json_experiment(self):
        ...
        self.expfilename = os.environ['DATA_BASE']+'/planner_experiments/%s_%s_%s.json' %(self.execution_date, self.best_primitive, execution_success_string)
...
if __name__ == '__main__':
    ...
    if not os.path.isdir(os.environ['DATA_BASE']+'/loopdata/planner/'):
        os.makedirs(os.environ['DATA_BASE']+'/loopdata/planner')
    if not os.path.isdir(os.environ['DATA_BASE']+'/planner_experiments'):
        os.makedirs(os.environ['DATA_BASE']+'/planner_experiments')
    if not os.path.isdir(os.environ['CODE_BASE']+'/output'):
        os.makedirs(os.environ['CODE_BASE']+'/output')
